...
public class Provider extends ContentProvider {
     ...
     */
    public static final class ToneAnalyser_Data implements AWAREColumns {
        public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY + "/" + DB_TBL_TEMPLATE);
        public static final String CONTENT_TYPE = ContentResolver.CURSOR_DIR_BASE_TYPE + "/vnd.com.aware.plugin.tone_analyser.provider.table_one"; //modify me
        public static final String CONTENT_ITEM_TYPE = ContentResolver.CURSOR_ITEM_BASE_TYPE + "/vnd.com.aware.plugin.tone_analyser.provider.table_one"; //modify me

        //Note: integers and strings don't need a type prefix_
        public static final String TONE = "tone_name";
    }
    ...
    public static String getAuthority(Context context) {
        AUTHORITY = context.getPackageName() + ".provider.tone_analyser";
        return AUTHORITY;
    }
    ...
    public boolean onCreate() {
        ...
        tableOneHash.put(ToneAnalyser_Data._ID, ToneAnalyser_Data._ID);
        tableOneHash.put(ToneAnalyser_Data.TIMESTAMP, ToneAnalyser_Data.TIMESTAMP);
        tableOneHash.put(ToneAnalyser_Data.DEVICE_ID, ToneAnalyser_Data.DEVICE_ID);
        tableOneHash.put(ToneAnalyser_Data.TONE, ToneAnalyser_Data.TONE);
...
/* Start from here*/
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case TABLE_ONE_DIR:
                long _id = database.insert(DATABASE_TABLES[0], ToneAnalyser_Data.DEVICE_ID, values);
                database.setTransactionSuccessful();
                database.endTransaction();
                if (_id > 0) {
                    Uri dataUri = ContentUris.withAppendedId(ToneAnalyser_Data.CONTENT_URI, _id);
    ...
    public String getType(Uri uri) {
        switch (sUriMatcher.match(uri)) {
            ...
            case TABLE_ONE_DIR:
                return ToneAnalyser_Data.CONTENT_TYPE;
            case TABLE_ONE_ITEM:
                return ToneAnalyser_Data.CONTENT_ITEM_TYPE;
